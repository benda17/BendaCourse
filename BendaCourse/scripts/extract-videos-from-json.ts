...
/**
 * Extract video URLs from lessons in sales-course-data.json
 * 
 * This script will:
 * 1. Load sales-course-data.json
...
async function extractVideos() {
  ...
  if (!fs.existsSync(dataPath)) {
    console.error('❌ sales-course-data.json not found!')
    console.error('   Please run the extraction script first or create the file manually.')
    process.exit(1)
  }
    ...
    for (const module of courseData.modules) {
      ...
      for (const lesson of module.lessons) {
        ...
        try {
          ...
          await new Promise(resolve => setTimeout(resolve, 3000)) // Wait for video to load
